...
const Dots = class Dots {
  constructor(canvasElement, width, height) {
    ...
    const grd_active = that.ctx.createLinearGradient(
      0,
      0,
      that.canvas.width,
      0
    ...
    for (
      let i = 0;
      i < Math.round((that.canvas.width / circlesMargin) * 1.5);
      i++
    ) {
      for (
        let j = 0;
        j < Math.round((that.canvas.height / circlesMargin) * 1.5);
        j++
      ) {
        that.circles.push({
  ...
  animatePoints(circle) {
    ...
    for (let i = 0; i < that.points.length; i++) {
      ...
      that.circles.forEach(function (circle) {
        ...
        that.drawCircles(
          (point.x + midpoint.x) / 2,
          (point.y + midpoint.y) / 2,
          circle,
          spreadRate
        );
        that.drawCircles(lastPoint.x, lastPoint.y, circle, spreadRate);
        that.drawCircles(
          (lastPoint.x + midpoint.x) / 2,
          (lastPoint.y + midpoint.y) / 2,
          circle,
          spreadRate
        );
      });
  ...
  drawCircles(x, y, circle, spreadRate) {
    ...
    if (dist < 8) {
      if (Math.floor(spreadRate)) circle.animation = true;
      else circle.animation = false;
...
new Dots(
  document.querySelector("#animation-dots"),
  window.innerWidth,
  window.innerHeight
);
...
function generateElectroCircle(count) {
  var currentPath = document.createElementNS(
  ...
  new TWEEN.Tween(percent)
    ...
    .onComplete(function () {
      ...
      setTimeout(function () {
        ...
        else generateElectroCircle(count + 1);
      }, 2000);
    })
    .start();
}
